...
│   └── run_sagemaker.py    # reference cells: fit -> deploy -> test -> delete
...
1. **Open SageMaker.** In the AWS Console search bar (top center), type `SageMaker` and click
   **Amazon SageMaker**.
2. **Create a notebook instance.** Left sidebar → **Notebook** → **Notebook instances** →
   **Create notebook instance**.
   - Name: anything, e.g. `fashion-mnist-cnn`.
   - Instance type: `ml.t3.medium` is enough and cheap (this only *launches* training/deploy
     jobs on other instances; it doesn't train on itself).
   - IAM role: choose **Create a new role** → accept the default (S3 bucket access) →
     **Create role**.
   - Click **Create notebook instance** and wait for status to change from `Pending` to
     `InService` (a few minutes).
3. **Open it.** Once `InService`, click **Open Jupyter** (or **Open JupyterLab**) on that row.
4. **Get this repo onto the instance.** In Jupyter: **New → Terminal**, then:
   ```bash
   git clone <this repo's URL>
   ```
   (or use Jupyter's **Upload** button to upload the `sagemaker/` and `src/` folders manually
   if the repo isn't pushed anywhere yet).
5. **Run the driver code.** Open a new notebook next to the cloned repo (kernel:
   `conda_pytorch_p310`), and copy each `# --- Cell N ---` block from
   [`sagemaker/run_sagemaker.py`](sagemaker/run_sagemaker.py) into its own cell, running them
   in order: setup → `estimator.fit()` (trains on a separate managed instance, streams logs
   live) → `estimator.deploy()` (creates the real-time endpoint) → test prediction →
